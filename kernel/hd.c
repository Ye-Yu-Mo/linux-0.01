  ...
  ((s1)->hd < (s2)->hd ||                                                      \
   (s1)->hd == (s2)->hd &&                                                     \
       ((s1)->cyl < (s2)->cyl ||                                               \
        (s1)->cyl == (s2)->cyl &&                                              \
            ((s1)->head < (s2)->head ||                                        \
             (s1)->head == (s2)->head && ((s1)->sector < (s2)->sector))))
...
void unexpected_hd_interrupt(void) { panic("Unexpected HD interrupt\n\r"); }
...
static void read_intr(void) {
  ...
  port_read(HD_DATA,
            this_request->bh->b_data + 512 * (this_request->nsector & 1), 256);
...
static void do_request(void) {
  ...
  if (this_request->cmd == WIN_WRITE) {
    hd_out(this_request->hd, this_request->nsector, this_request->sector,
           this_request->head, this_request->cyl, this_request->cmd,
           &write_intr);
